...
class BClassifier(nn.Module):
    def __init__(self, input_size, output_class, fold, dropout_v=0.0, nonlinear=True, passing_v=False, confounder_path=False, confounder_merge='sub'): # K, L, N
        ...
        if confounder_path:
            self.confounder_path = confounder_path
            print('Deconfounding')
            print(f"confounder_path type: {type(confounder_path)}") 
            confounder_file = f"5_cls_agnostic_fold{fold}.npy"
            selected_conf_path = os.path.join(confounder_path, confounder_file)
            self.confounder_path = selected_conf_path
            print(f"Loading confounder from: {selected_conf_path}")
            print(f'Using confounder_merge : {self.confounder_merge} ')
            conf_list = []
            conf_tensor = torch.from_numpy(np.load(selected_conf_path)).view(-1, input_size).float()
            ...
            if confounder_merge == 'sub':
                self.fcc =  self.fcc = nn.Conv1d(output_class, output_class, kernel_size=input_size) 
            else:
    ...
    def forward(self, feats, c): # N x K, N x C
        ...
        #print('B before', B.shape)
        # cls-specific confounder
        if self.confounder_path:
            if 'agnostic' in os.path.basename(self.confounder_path):
                device = B.device
                ...
                #print('Conf_feats', conf_feats.shape)
                if self.confounder_merge == 'sub':
                    B = B - conf_feats # 1x1x1025
                    #print('B', B.shape)
                else:
